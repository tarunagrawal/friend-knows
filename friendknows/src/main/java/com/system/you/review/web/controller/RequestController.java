...
public class RequestController extends ControllerSupport {
	...
	public ModelAndView create(Model model, RequestFormBean formBean)
			throws UIException {
	...
	public ModelAndView close(@PathVariable String requestId, Model model)
			throws UIException {
		RequestContext<String, String> requestContext = closeHelper
				.close(requestId);
		requestContext.setModel(model);
	...
	public ModelAndView addReviewers(@RequestParam String requestId,
			@RequestParam String friends, Model model) throws UIException {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("adding reviewer for "
					+ getRequestor().getUser().getName() + ", for request id:"
					+ requestId);
			LOGGER.debug("input friends:" + friends);
		}
		RequestContext<String[], Set<ReviewerViewBean>> requestContext = addReviewerHelper
				.addReviewers(requestId, friends);
		String successViewName = "addReviewerSuccess";
		String errorView = "error";
		requestContext.setSuccessView(successViewName);
		requestContext.setErrorView(errorView);
		requestContext.setModel(model);
		return handleResponse(requestContext, new UIExceptionFactory() {
			@Override
	...
	public ModelAndView removeReviewer(@PathVariable String requestId,
		...
		RequestContext<String[], String> requestContext = removeReviewerHelper
	...
	public ModelAndView editDescription(@PathVariable String requestId,
			...
			throws UIException {
		RequestContext<String[], String> requestContext = editDescriptionHelper
	...
	@ExceptionHandler(value = { CreateRequestException.class,
			...
			RequestForwardException.class, UIException.class })
